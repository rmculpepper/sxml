...
(define (ddo:ancestor test-pred? [num-anc 0])
  (lambda (node)   ; node or nodeset
    (let loop ([src (as-nodeset node)]
               [prev-ancestors '()]
               [res '()])
      (cond [(null? src)  ; everyone processed
             (reverse res)]
            [else
             (let ((curr (car src)))
               (cond [(sxml:context? curr)
                      (let rpt ((curr-ancs (reverse (sxml:context->ancestors-u curr)))
                                (dupl '()))
                        (cond
...
(define (ddo:ancestor-or-self test-pred? [num-anc 0])
  (lambda (node)   ; node or nodeset
    (let loop ((src (as-nodeset node))
               (prev-ancestors '())
               (res '()))
      (cond [(null? src)
             ...
             (reverse res)]
            [else
             (let rpt ((curr-ancs (reverse (sxml:context->content (car src))))
                       (dupl '()))
               (cond
                 [(null? curr-ancs)  ; no new ancestors
                  (loop (cdr src) prev-ancestors res)]
...
(define (ddo:child test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define (child4this curr-node src)
    ; Arguments
             ...
             (cond
               ...
               [else  ; processing the current res-candidate
                (let rpt ((more (list (car res-candidates)))
                          ...
                          (src src)
                          (res
                           (cond [(test-pred? (car res-candidates))
                                  (append
                                   res
                                   (list
                                    (draft:smart-make-context
  ...
  (lambda (node)   ; node or nodeset
    (cond [(nodeset? node)
          ...
          [else ((draft:child test-pred? num-anc) node)])))
...
; order
(define (ddo:descendant test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define desc (draft:descendant test-pred? num-anc))
  (lambda (node)   ; node or nodeset
    (let loop ((src (as-nodeset node))
               (next-node #f)
               (content-to-scan '())
               (res '()))
      (cond [(null? content-to-scan)
             (cond
            ...
            [else
             (let ((curr-cntnt (car content-to-scan)))
               (call-with-values
                (lambda ()
                  (cond [(eq? (car curr-cntnt) next-node)
                ...
                (lambda (new-src new-next)
                  (loop new-src
                        ...
                        (append
                         ...
                         (cdr content-to-scan))
                        (cond [(test-pred? (car curr-cntnt)) ; satisfies the node test
                               (cons
                                (draft:smart-make-context
...
; document order
(define (ddo:descendant-or-self test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define desc-os (draft:descendant-or-self test-pred? num-anc))
  (lambda (node)   ; node or nodeset
    (let loop ((src (as-nodeset node))
               (next-node #f)
               (content-to-scan '())
...
(define (ddo:following test-pred? [num-anc 0])
  (let ((child (sxml:child sxml:node?))
        (foll (draft:following test-pred? num-anc)))
    (lambda (node)   ; node or nodeset
      (cond
...
(define (ddo:following-sibling test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define (all-following-siblings node)
    ; not a nodeset
    (cond [(and (sxml:context? node)                  
               ...
               (cond [(null? nset) res]
                     [else
                      (call-with-values
                       ...
                       (lambda (add-res new-nset)
                         (iter new-nset (append res add-res))))]))]
            [else ((draft:following-sibling test-pred? num-anc) node)]))))
...
(define (ddo:parent test-pred? [num-anc 0])
  (lambda (node)   ; node or nodeset
    (let loop ([src (as-nodeset node)]
               [prev-parents '()]
               [res '()])
      (cond [(null? src) (reverse res)]
            [else
             (let ((curr (car src)))
               (cond [(and (sxml:context? curr)
                           (not (null? (sxml:context->ancestors-u curr))))
                      (let ((curr-parent (car (sxml:context->ancestors-u curr))))
                        (cond [(memq curr-parent prev-parents)
                               ; a duplicate node               
...
(define (ddo:preceding test-pred? [num-anc 0])
  (let ((prec (draft:preceding test-pred? num-anc)))
    (lambda (node)   ; node or nodeset
      (cond
        [(null? node)  ; empty nodeset - nothing to do
         '()]        
...
(define (ddo:preceding-sibling test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define (all-preceding-siblings node) ; not a nodeset
...
; nodeset
(define (ddo:following-single-level test-pred? [num-anc 0])
  (let ((foll (draft:following test-pred? num-anc)))
    (lambda (node)   ; node or nodeset
      (cond
        [(null? node)  ; empty nodeset - nothing to do
         '()]
...
(define (ddo:following-sibling-single-level test-pred? [num-anc 0])
  (lambda (node)   ; node or nodeset
    (let loop ((src (as-nodeset node))    
               (res '()))
      (cond [(null? src)
...
; ddo:parent
(define (ddo:parent-single-level test-pred? [num-anc 0])
  (lambda (node)   ; node or nodeset
    (let loop ((src (as-nodeset node))
               (prev-parent #f)
               (res '()))
            ...
            [else
             (let ((curr (car src)))
               (cond [(and (sxml:context? curr)
                           (not (null? (sxml:context->ancestors-u curr))))
                      (let ((curr-parent (car (sxml:context->ancestors-u curr))))
                        (cond [(eq? curr-parent prev-parent)
...
; input nodeset
(define (ddo:preceding-single-level test-pred? [num-anc 0])
  (let ((prec (draft:preceding test-pred? num-anc)))
    (lambda (node)   ; node or nodeset
      (cond
        [(null? node)  ; empty nodeset - nothing to do
...
(define (ddo:preceding-sibling-single-level test-pred? [num-anc 0])
  (lambda (node)   ; node or nodeset
...
(define (ddo:ancestor-pos test-pred? [num-anc 0])
  (define (assmemq key lst) ; A hybrid of assq and memq
    (cond
      [(null? lst) #f]
      [(eq? key (caar lst)) lst]
                             ...
                             (lambda (prev-ancestors ancs-alist this-nset)
                               (let creat ((prev-ancestors prev-ancestors)
                                 ...
                                 (cond [(null? new-content)
                                       ...
                                       [else
                                        (let ((new-this-nset
                                               (cond [(test-pred? (caar new-content))
                                                      ; add to the result
                                                      (cons
                                                       (cons
                                                        (draft:smart-make-context
...
(define (ddo:ancestor-or-self-pos test-pred? [num-anc 0])
  (define (assmemq key lst)
                       ...
                       (call-with-values
                        ...
                        (lambda (prev-ancestors ancs-alist this-nset)
                          (let creat ((prev-ancestors prev-ancestors)
                                      ...
                                      (this-nset this-nset)
                                      (new-content new-content))
                            (cond [(null? new-content) ; everyone processed
                                   (loop (cdr src)
                                         prev-ancestors
                                         ancs-alist
                                         (cons this-nset pos-res)
...
(define (ddo:child-pos test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define (create-pos-nset nset ancestors vacant-num) ; Creates a pos-nodeset
    (cond [(null? nset) '()]
          [else
           (cons
  ...
  (define (src-walk curr-node src order-num)
    ...
    (let ((curr-children
           (child (sxml:context->node curr-node))))
      (cond [(null? curr-children)
             ; nothing to do for this curr-node
             (values '() src order-num)]
            [else
             (let ((curr-ancestors (draft:list-head
                                    (sxml:context->content curr-node)
                                    num-anc)))
               (cond [(null? src) ; no searching for descendants required
                     ...
                     [else
                      (let loop ((src src)
                                 ...
                                 (this-res
                                  (cond [(test-pred? (car curr-children))
                                         (list
                                          (cons
                          ...
                          [(eq? (car desc-to-scan) next-node)
                           (call-with-values
                            ...
                            (lambda (new-pos-res new-src new-order-num)
                              (cond [(null? new-src) ; no more nodes in src nodeset
                                     (values
                                      (cons
                                       (append
                                        (reverse this-res)
  ...
  (lambda (node)   ; node or nodeset
    (let rpt ((src (as-nodeset node))
              (pos-result '())
              (order-num 1))
      (cond [(null? src) ; all processed
             (remove-empty-nodesets pos-result)]
            [else
             (call-with-values
...
(define (ddo:descendant-pos test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define desc (draft:descendant test-pred? num-anc))
  (define (src-walk curr-node src order-num)
    ; curr-node - current input node (probably context)
               ...
               (cond [(eq? (car curr-cntnt) next-node)
                      (call-with-values
                       ...
                       (lambda (new-pos-res new-src new-order-num)
                         (loop new-src
                               (cond [(null? new-src) #f]
                                     [else (sxml:context->node (car new-src))])
                               (cdr content-to-scan)  ; descendants processed
                               (append
                                (reverse (car new-pos-res))
                                (cond [(test-pred? (car curr-cntnt)) ; this res
                                       (cons
                                        (cons
                                         (draft:smart-make-context
                                          (car curr-cntnt) (cdr curr-cntnt) num-anc)
                      ...
                      (loop src
                            ...
                            (append  ; content-to-scan
                             (map
                              ...
                              (child (car curr-cntnt)))
                             (cdr content-to-scan))
                            (cond [(test-pred? (car curr-cntnt)) ; this res satisfies the node test
                                   (cons
                                    (cons
                                     (draft:smart-make-context
                                      (car curr-cntnt) (cdr curr-cntnt) num-anc)
...
; When no node in the input node-set is a descendant of another node in the
; input node-set, the ordinary draft:descendant function can be used
(define (ddo:descendant-or-self-pos test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define (src-walk curr-node src order-num)
    ; curr-node - current input node (probably context)
    ; src - the remaining nodes in the input nodeset
...
(define (ddo:following-sibling-pos test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define (associate-num nset ancestors vacant-num)
    (cond [(null? nset) nset]
          [else
           (cons
            (cons
  ...
  (define (process-single curr src vacant-num)
                  ...
                  (let loop ([foll-siblings (cdr foll-siblings)]
                    ...
                    (cond
                      ...
                      [(null? descs)  ; descendants of current foll-sibling
                       (cond [(null? foll-siblings) ; that stuff is over
...
; single result node
(define (ddo:parent-pos test-pred? [num-anc 0])
  (let ((parent (ddo:parent test-pred? num-anc)))
    (define (add-order-num num nset)
      (if (null? nset)
          nset
...
(define (ddo:preceding-sibling-pos test-pred? [num-anc 0])
  (define child (sxml:child sxml:node?))
  (define (reverse-desc node)
    ; reverse descendants of node
    (let scan ((more (ddo:attr-child node))
               (res '()))
      (cond [(null? more) ; result formed
             res]
            [else
  ...
  (define (associate-num nset ancestors vacant-num)
    (if (null? nset)
        nset
        (cons
         (cons (cond [(null? ancestors) (car nset)]
                     [else (draft:make-context (car nset) ancestors)])
               vacant-num)
         (associate-num (cdr nset) ancestors (- vacant-num 1)))))
  (define (process-single curr src vacant-num)
    ; curr - current context to be processed
...
(define (ddo:following-single-level-pos test-pred? [num-anc 0])
  (define descend (draft:descendant-or-self test-pred? num-anc))
  (define follow (draft:following test-pred? num-anc))
  (define (process-single curr src vacant-num)
    ; curr - current context to be processed
    ; src - remaining source contexts
    ; vacant-num - order number for the result node
             ...
             [else  ; go on to scan the next node
              (loop
               ...
               (append
                ...
                (cdr content-set))
               (cond [(test-pred? (caar content-set))
  ...
  (lambda (node)  ; node or nodeset
    (let ((nset (as-nodeset node)))
      (if (null? nset)  ; nothing to be done
          nset
          (remove-empty-nodesets
           (process-single (car nset) (cdr nset) 1))))))

; Following-sibling axis for the special case
(define (ddo:following-sibling-single-level-pos test-pred? [num-anc 0])
  (define (process-single curr src vacant-num)
    ; curr - current context to be processed
    ; src - remaining source contexts
    ; vacant-num - order number for the result node
    ...
    (cond [(or (not (sxml:context? curr))
               (null? (sxml:context->ancestors-u curr)))
           ; Siblings cannot be identified
           (cond [(null? src) ; recursion is over
                  '()]
                 [else (process-single (car src) (cdr src) vacant-num)])]
          [else
           (cond
             [(memq (sxml:context->node-u curr)
                    (cdr  ; parent is an element => cdr gives its children
                   ...
                   (let ((ancestors
                     ...
                     (cond [(null? src)
                            ...
                            (let no-more ((foll-siblings (cdr foll-siblings))
                                          (res '())
                                          (vacant-num vacant-num))
                              (cond [(null? foll-siblings) ; everyone processed
                                     (list (reverse res))]
                                    [else
                                     (no-more
                                      (cdr foll-siblings)
                            ...
                            (let ((next (sxml:context->node (car src))))
                              (let more ([foll-siblings (cdr foll-siblings)]
                                ...
                                (cond [(null? foll-siblings)
                                      ...
                                      [else
                                       (let ((new-res
                                              (cond [(test-pred? (car foll-siblings))
                                                     (cons
                                                      (cons
                                                       (if (null? ancestors)
                                                           (car foll-siblings)
                                                      ...
                                                      res)]
                                                    [else res])))
                                         (cond [(eq? (car foll-siblings) next) ; recursion
                                                (let ((pos-res
                                                       (process-single
                                                        (car src)
                                                        (cdr src)
                                                        (+ vacant-num 1))))
                                                  (cond [(null? pos-res) ; this shouldn't occur
  ...
  (lambda (node)  ; node or nodeset
    (let ((nset (as-nodeset node)))
      (cond [(null? nset) ; nothing to be done
             nset]
            [else
             (remove-empty-nodesets
              (process-single (car nset) (cdr nset) 1))]))))
...
(define (ddo:parent-single-level-pos test-pred? [num-anc 0])
  (let ((parent (ddo:parent-single-level test-pred? num-anc)))
    (define (add-order-num num nset)
      (if (null? nset)
          nset
          (cons (list (cons (car nset) num))
...
; Preceding axis for the special case
(define (ddo:preceding-single-level-pos test-pred? [num-anc 0])
  (define descend (draft:descendant-or-self test-pred? num-anc))
  (define precede (draft:preceding test-pred? num-anc))
  (define (process-single curr src vacant-num)
    ; curr - current context to be processed
    ; src - remaining source contexts
    ; vacant-num - order number for the result node
...
; Preceding-sibling axis for the special case
(define (ddo:preceding-sibling-single-level-pos test-pred? [num-anc 0])
  (define (process-single curr src vacant-num)
    ; curr - current context to be processed
    ; src - remaining source contexts
    ; vacant-num - order number for the result node
             ...
             [(memq (sxml:context->node-u curr)
              ...
              => (lambda (prec-siblings)
                   (let ((ancestors
                          (draft:list-head
                           (sxml:context->ancestors-u curr) num-anc)))
                     (cond [(null? src) ; no more nodes
                            (let no-more ((prec-siblings (cdr prec-siblings))
                                          (res '())
                                          (vacant-num vacant-num))
                              (cond [(null? prec-siblings) ; everyone processed
                                     (list (reverse res))]
                                    [else
                                     (no-more
                                      (cdr prec-siblings)
                                      (cond [(test-pred? (car prec-siblings))
                                             (cons
                                              (cons
                                               (if (null? ancestors)
                                                   (car prec-siblings)
                                            ...
                                            [else res])
                                      (- vacant-num 1))]))]
                           [else ; else there are more nodes in src
                            (let ((next (sxml:context->node (car src))))
                                       ...
                                       (let ((new-res
                                              (cond [(test-pred? (car prec-siblings))
                                                     (cons
                                                      ...
                                                      res)]
                                                    [else res])))
                                         (cond [(eq? (car prec-siblings) next) ; recursion
                                                (let ((pos-res
                                                       (process-single
                                                        (car src)
                                                        (cdr src)
...
;; remove empty result nodesets
(define (remove-empty-nodesets vs)
  (filter (lambda (x) (not (null? x))) vs))
